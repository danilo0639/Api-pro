...
export default function CustomPaginationActionsTable() {
  ...
  const emptyRows =
    page > 0 ? Math.max(0, (1 + page) * rowsPerPage - data.entries.length) : 0;
  ...
  return (
    <TableContainer component={Paper}>
      ...
      <TableHead>
          <TableRow>
            <TableCell align="left">Api data</TableCell>
        ...
        <TableBody>
          {(rowsPerPage > 0
            ? data.entries.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage)
            : data.entries
          ).map((data) => (
            <TableRow
              key={data.entries}
